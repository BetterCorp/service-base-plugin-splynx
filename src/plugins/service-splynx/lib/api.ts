...
export class SplynxApi {
  ...
  validateLoginData(auth_data: any) {
    ...
    if (auth_data["auth_type"] === LOGIN_TYPE_API_KEY) {
      ...
      auth_data["auth_type"] === LOGIN_TYPE_ADMIN ||
      auth_data["auth_type"] === LOGIN_TYPE_CUSTOMER
    ) {
      required.push("login");
    ...
    for (var i = 0; i < required.length; i++) {
      if (!(required[i] in auth_data)) {
        throw new Error(
          "Required auth param `" + required[i] + "` is missing!"
        );
      }
    }
  }
  ...
  fixSettingsForUploadingFiles(settings: any) {
    ...
    if (settings.path === "admin/support/ticket-messages") {
      ...
      if (
        ...
        typeof settings.params.files !== "undefined"
      ) {
        // ... with attachments
    ...
    if (settings.path.search(documentsRe) != -1) {
      ...
      if (
        typeof settings.params !== "undefined" &&
        typeof settings.params.file !== "undefined"
      ) {
  ...
  makeRequest(settings: any, callback: any) {
    ...
    request(options, (err: any, res: any, body: any) => {
      ...
      if (
        res.statusCode == "401" &&
        typeof this.on_unauthorized_callback === "function"
      ) {
